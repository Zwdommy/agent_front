<template>
  <div class="product-scene-container">
    <canvas ref="backgroundCanvas" id="productBg"></canvas>
    <canvas ref="animationCanvas" id="productCatCanvas" @click="handleCanvasClick"></canvas>
      ...
      <div class="bubble-content" ref="chatMessages">
        <div class="message system">
          <div class="message-content">
            <p>欢迎来到产品中心！我是产品助手，有什么产品问题需要了解吗？</p>
          </div>
        </div>
      </div>
...
const updatePosition = () => {
  ...
  if (moving) {
    ...
    if (keyboardMoving) {
      console.log(`产品场景 - 小猫位置: X=${Math.round(posX)}, Y=${Math.round(posY)}`)
    }
  }
...
const checkProductCollision = () => {
  ...
  // 中心区域：返回主场景的传送门 (向右偏移100px，向下偏移70px)
  const centerAreaWidth = 150
  const centerAreaHeight = 150
  const centerX1 = worldWidth / 2 - centerAreaWidth / 2 + 300
  const centerX2 = worldWidth / 2 + centerAreaWidth / 2 + 300
  const centerY1 = worldHeight / 2 - centerAreaHeight / 2 + 70
  const centerY2 = worldHeight / 2 + centerAreaHeight / 2 + 70
  ...
  if (keyState.ArrowUp || keyState.ArrowDown || keyState.ArrowLeft || keyState.ArrowRight) {
    ...
    console.log(`中心返回区域: X(${Math.round(centerX1)}~${Math.round(centerX2)}), Y(${Math.round(centerY1)}~${Math.round(centerY2)})`)
  }
...
const handleCanvasClick = (e: MouseEvent) => {
  ...
  console.log(`产品场景 - 点击移动到: X=${Math.round(targetX)}, Y=${Math.round(targetY)}`)
}
...
const sendMessage = async () => {
  ...
  try {
    ...
    removeLoadingIndicator()
    addMessage('system', 'AI产品服务暂时不可用，请稍后再试。')
  }
}
...
const addLoadingIndicator = () => {
  ...
  const contentDiv = document.createElement('div')
  contentDiv.className = 'message-content'
  contentDiv.textContent = '产品助手思考中...'
...
const initBackgroundImage = () => {
  bgImg = new Image()
  import('@/assets/static/product_scence/site-4.png').then(module => {
    bgImg.src = module.default
    bgImg.onload = () => {
      console.log('✅ 产品场景背景图片加载完成')
      checkAllLoaded()
    }
  }).catch(() => {
    console.error('❌ Failed to load site-4.png')
  })
}
