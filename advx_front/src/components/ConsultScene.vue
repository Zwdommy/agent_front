<template>
  <div class="consult-scene-container">
    <canvas ref="backgroundCanvas" id="consultBg"></canvas>
    <canvas ref="animationCanvas" id="consultCatCanvas" @click="handleCanvasClick"></canvas>
      ...
      <div class="bubble-content" ref="chatMessages">
        <div class="message system">
          <div class="message-content">
            <p>欢迎来到咨询中心！我是咨询助手，有什么需要咨询的问题吗？</p>
          </div>
        </div>
      </div>
...
const updatePosition = () => {
  ...
  if (moving) {
    ...
    if (keyboardMoving) {
      console.log(`咨询场景 - 小猫位置: X=${Math.round(posX)}, Y=${Math.round(posY)}`)
    }
  }
...
const checkConsultCollision = () => {
  ...
  if (keyState.ArrowUp || keyState.ArrowDown || keyState.ArrowLeft || keyState.ArrowRight) {
    console.log(`咨询场景 - 小猫位置: X=${Math.round(posX)}, Y=${Math.round(posY)}`)
    console.log(`返回主场景区域: X(${exitX1}~${exitX2}), Y(${exitY1}~${exitY2})`)
  }
...
const handleCanvasClick = (e: MouseEvent) => {
  ...
  console.log(`咨询场景 - 点击移动到: X=${Math.round(targetX)}, Y=${Math.round(targetY)}`)
}
...
const sendMessage = async () => {
  ...
  try {
    ...
    removeLoadingIndicator()
    addMessage('system', 'AI咨询服务暂时不可用，请稍后再试。')
  }
}
...
const initBackgroundImage = () => {
  bgImg = new Image()
  import('@/assets/static/askScence/site-3.png').then(module => {
    bgImg.src = module.default
    bgImg.onload = () => {
      console.log('✅ 咨询场景背景图片加载完成')
      checkAllLoaded()
    }
  }).catch(() => {
    console.error('❌ Failed to load site-3.png')
  })
}
