<template>
  <div class="management-scene-container">
    <canvas ref="backgroundCanvas" id="managementBg"></canvas>
    <canvas ref="animationCanvas" id="managementCatCanvas" @click="handleCanvasClick"></canvas>
...
const updatePosition = () => {
  ...
  if (moving) {
    ...
    if (keyboardMoving) {
      console.log(`管理场景 - 小猫位置: X=${Math.round(posX)}, Y=${Math.round(posY)}`)
    }
  }
...
const checkManagementCollision = () => {
  // 管理场景的特殊碰撞检测区域
  // 左下角区域：返回主场景的传送门
  
  // 计算左下角区域的坐标（基于世界尺寸的相对位置）
  ...
  const exitY1 = worldHeight - exitAreaHeight - margin
  const exitY2 = worldHeight - margin

  // 调试信息 - 显示小猫位置和区域坐标
  if (keyState.ArrowUp || keyState.ArrowDown || keyState.ArrowLeft || keyState.ArrowRight) {
    console.log(`管理场景 - 小猫位置: X=${Math.round(posX)}, Y=${Math.round(posY)}`)
    console.log(`返回主场景区域: X(${exitX1}~${exitX2}), Y(${Math.round(exitY1)}~${Math.round(exitY2)})`)
  }

  // 检测是否在返回主场景的区域内
  if (posX >= exitX1 && posX <= exitX2 && posY >= exitY1 && posY <= exitY2) {
    console.log('🚪 检测到传送门！准备返回主场景')
...
const handleCanvasClick = (e: MouseEvent) => {
  ...
  console.log(`管理场景 - 点击移动到: X=${Math.round(targetX)}, Y=${Math.round(targetY)}`)
}
...
const initBackgroundImage = () => {
  bgImg = new Image()
  import('@/assets/static/Manage_Scence/site-5.png').then(module => {
    bgImg.src = module.default
    bgImg.onload = () => {
      console.log('✅ 管理场景背景图片加载完成')
      checkAllLoaded()
    }
  }).catch(() => {
    console.error('❌ Failed to load site-5.png')
  })
}
...
#managementBg, #managementCatCanvas {
  position: absolute;
  top: 0;
  left: 0;
  width: 100vw;
