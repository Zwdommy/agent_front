<template>
  <div class="promotion-scene-container">
    <canvas ref="backgroundCanvas" id="promotionBg"></canvas>
    <canvas ref="animationCanvas" id="promotionCatCanvas" @click="handleCanvasClick"></canvas>
      ...
      <div class="bubble-content" ref="chatMessages">
        <div class="message system">
          <div class="message-content">
            <p>欢迎来到宣传中心！我是宣传助手，有什么宣传活动需要了解吗？</p>
          </div>
        </div>
      </div>
...
const updatePosition = () => {
  ...
  if (moving) {
    ...
    if (keyboardMoving) {
      console.log(`宣传场景 - 小猫位置: X=${Math.round(posX)}, Y=${Math.round(posY)}`)
    }
  }
...
const checkPromotionCollision = () => {
  ...
  // 中心偏右上区域：返回主场景的传送门 (小猫出生坐标右移200px，上移200px)
  const centerAreaWidth = 150
  const centerAreaHeight = 150
  const centerX1 = worldWidth / 2 + 200
  const centerX2 = centerX1 + centerAreaWidth
  const centerY1 = worldHeight / 2 - 200
  ...
  if (keyState.ArrowUp || keyState.ArrowDown || keyState.ArrowLeft || keyState.ArrowRight) {
    ...
    console.log(`中心偏右上返回区域: X(${Math.round(centerX1)}~${Math.round(centerX2)}), Y(${Math.round(centerY1)}~${Math.round(centerY2)})`)
  }
...
const handleCanvasClick = (e: MouseEvent) => {
  ...
  console.log(`宣传场景 - 点击移动到: X=${Math.round(targetX)}, Y=${Math.round(targetY)}`)
}
...
const sendMessage = async () => {
  ...
  try {
    ...
    removeLoadingIndicator()
    addMessage('system', 'AI宣传服务暂时不可用，请稍后再试。')
  }
}
...
const addLoadingIndicator = () => {
  ...
  const contentDiv = document.createElement('div')
  contentDiv.className = 'message-content'
  contentDiv.textContent = '宣传助手思考中...'
...
const initBackgroundImage = () => {
  bgImg = new Image()
  import('@/assets/static/advertise_scence/site-2.png').then(module => {
    bgImg.src = module.default
    bgImg.onload = () => {
      console.log('✅ 宣传场景背景图片加载完成')
      checkAllLoaded()
    }
  }).catch(() => {
    console.error('❌ Failed to load site-2.png')
  })
}
...
#promotionBg, #promotionCatCanvas {
  position: absolute;
  top: 0;
  left: 0;
  width: 100vw;
